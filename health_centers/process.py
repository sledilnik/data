...
for sheet in sheets:
    expected = [  # header row 1
        ...
        'Št. sumov na COVID brez pregleda (triaža po telefonu)', 'Št. opravljenih testiranj COVID',
        'Št.  pozitivnih COVID', 'Št. napotitev v bolnišnico', 'Št. napotitev v samoosamitev', 'Opombe'
    ]
    actual = [cell.value for cell in sheet[1]]
    assert actual[:len(expected)] == expected, (sheet, actual)
    expected = [None, None, 1, 2, 3, 4, 5, 6, 7, 8]  # header row 2
    actual = [cell.value for cell in sheet[2]]
    assert actual[:len(expected)] == expected, (sheet, actual)
...
with open(args.file, 'w', newline='') as csvfile:
    ...
    # write data
    dates = sorted(list(set([entity.date for entity in entities])), reverse=True)
    for date in dates:
